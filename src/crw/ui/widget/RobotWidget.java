 ...
 * @author nbb
 */
public class RobotWidget implements MarkupComponentWidget, WorldWindWidgetInt, ProxyServerListenerInt, ObserverServerListenerInt, ObservationListenerInt {
...
//        markerLayer.setElevation(10d);
		...
		protected void initButtons() {
				...
				batteryLabel.setFont(new java.awt.Font("Lucida Grande", 1, 13));
				batteryLabel.setOpaque(true);
				controlModeP.add(batteryLabel);
		...
		public void selectMarker(Marker boatMarker) {
            ...
            // Remove marker from renderables list and re-add it so it is rendered last 
				...
				if (boatMarker != null) {
						...
						proxyNameLabel.setText(" " + boatProxy.toString() + " ");
...
//
		...
		public void newObservation(Observation o) {
				// Only care about battery readings from the currently selected proxy
				//@todo store latest battery reading for each boat
				//@todo color code battery level?
				if (o.variable.equals(VehicleServer.SensorType.BATTERY.toString())
								&& selectedProxy != null
								&& o.source.equals(selectedProxy.getProxyName())) {
						batteryLabel.setText(String.format(" %.1f V", o.value));
				}
		}
}
